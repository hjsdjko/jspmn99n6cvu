...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">论坛帖子管理</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-two"><span>论坛帖子列表</span></li>
			</ol>
		</div>
		<!-- /Breadcrumb -->
		...
		<div id="center-container">
			...
			<div class="center-form">
				<div class="form-item">
					<label>动漫主题</label>
					<input type="text" id="dongmanzhutiSearch" class="form-control form-control-sm" placeholder="请输入动漫主题" aria-controls="tableId">
				</div>
				<div class="form-item">
					<label>用户账号</label>
			...
			<div class="btns">
				...
				<button onclick="chartDialog1()" type="button" class="btn btn-info btn-statis" v-if="crossBtnControl('分类占比')"><i class="fa fa-database"></i><span>分类占比</span></button>
				<button onclick="chartDialog2()" type="button" class="btn btn-info btn-statis" v-if="crossBtnControl('发贴统计')"><i class="fa fa-database"></i><span>发贴统计</span></button>
			</div>
			...
			<table id="tableId" class="table table-bordered">
				<thead>
					<tr>
						...
						<th onclick="sort('dongmanzhuti')">动漫主题<i id="dongmanzhutiIcon" class="fa fa-sort"></i></th>
						<th onclick="sort('tiezifenlei')">帖子分类<i id="tiezifenleiIcon" class="fa fa-sort"></i></th>
						<th>封面</th>
            ...
            <div class="openBox" v-if="showType1">
                ...
                <div class="openView">
                    <div id="tiezifenleiMain1" class="graph" style="width: 70vw;height:50vh;"></div>
                </div>
            </div>
            <div class="openBox" v-if="showType2">
                <div class="openBG" @click="showClick2"></div>
                <div class="openView">
                    <div id="yonghuxingmingMain2" class="graph" style="width: 70vw;height:50vh;"></div>
                </div>
            </div>
		</div>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#dongmanzhutiSearch').val() != null && $('#dongmanzhutiSearch').val() != ''){
				searchForm.dongmanzhuti ="%" +  $('#dongmanzhutiSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("luntantiezi/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				dongmanzhuti : searchForm.dongmanzhuti,	
		...
		function setDataRow(item,number){
			...
			var dongmanzhutiCell  = document.createElement('td');

						 dongmanzhutiCell.innerHTML = item.dongmanzhuti;

			row.appendChild(dongmanzhutiCell);
			var tiezifenleiCell  = document.createElement('td');

						 tiezifenleiCell.innerHTML = item.tiezifenlei;

			row.appendChild(tiezifenleiCell);
		...
		function sh(){
			...
			httpJson("luntantiezi/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("luntantiezi/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function discussList(id) {
            ...
            window.location.href = "../discussluntantiezi/list.jsp"
        }
		//清除会重复渲染的节点
        function clear(){
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"luntantiezi");
                        ...
                        success: function(data){
                            if(data.code == 0){
                                if(data.data != null){
                                    ...
                                    var myChart = echarts.init(document.getElementById('tiezifenleiMain1'),'macarons');
                                    var option = {};
                                    option = {
                                            title: {
                                                text: '分类占比',
                        ...
                        success: function(data){
                            if(data.code == 0){
                                if(data.data != null){
                                    ...
                                    var myChart = echarts.init(document.getElementById('yonghuxingmingMain2'),'macarons');
                                    var option = {};
                                        option = {
                                            title: {
                                                text: '发贴统计',
