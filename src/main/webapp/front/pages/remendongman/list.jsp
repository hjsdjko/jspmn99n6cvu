			...
			<div class="recommend" :style='{"width":"100%","margin":"10px auto","position":"relative","background":"none"}'>
				...
				<form class="layui-form filter" :style='{"padding":"20px 10px","alignItems":"center","flexWrap":"wrap","background":"#f5f5f5","display":"flex","width":"100%","justifyContent":"center","height":"auto"}'>
					...
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>动漫名称</div>
						<input type="text" :style='{"border":"1px solid #d2e9f6","padding":"0 10px","boxShadow":"inset 0px 0px 24px 0px rgba(48,134,185,.2)","margin":"0","outline":"none","color":"#333","borderRadius":"4px","width":"140px","fontSize":"14px","lineHeight":"40px","height":"40px"}' name="dongmanmingcheng" id="dongmanmingcheng" placeholder="动漫名称" autocomplete="off" class="layui-input">
					</div>
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>动漫作者</div>
						<input type="text" :style='{"border":"1px solid #d2e9f6","padding":"0 10px","boxShadow":"inset 0px 0px 24px 0px rgba(48,134,185,.2)","margin":"0","outline":"none","color":"#333","borderRadius":"4px","width":"140px","fontSize":"14px","lineHeight":"40px","height":"40px"}' name="dongmanzuozhe" id="dongmanzuozhe" placeholder="动漫作者" autocomplete="off" class="layui-input">
					</div>
				...
				<div :style='{"margin":"20px 0 10px","background":"none"}' class="lists">
					...
					<div :style='{"padding":"0 0px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}' class="list list-2">
						<div :style='{"padding":"10px","boxShadow":"0px 0px 4px #eee","margin":"0 0 20px","borderColor":"#eee","display":"flex","justifyContent":"space-between","flexWrap":"wrap","background":"rgba(255,255,255,1)","borderWidth":"1px","width":"49%","position":"relative","borderStyle":"solid","height":"400px"}' @click="jump('../remendongman/detail.jsp?id='+item.id)" v-for="(item,index) in dataList" :key="index" class="list-item animation-box">
							<img :style='{"cursor":"pointer","verticalAlign":"middle","padding":"10px","borderColor":"#9dcde9","objectFit":"cover","borderWidth":"6px","display":"inline-block","width":"48%","borderStyle":"dotted","height":"100%"}' :src="item.dongmantupian?baseurl+item.dongmantupian.split(',')[0]:''">
							<div :style='{"width":"48%","padding":"0px 10px","verticalAlign":"middle","overflow":"hidden","display":"inline-block","height":"100%"}' class="item-info">
								<div :style='{"cursor":"pointer","padding":"0 10px","margin":"0 0 10px","borderColor":"#9dcde9","whiteSpace":"nowrap","color":"#333","overflow":"hidden","background":"none","borderWidth":"0 0 2px","lineHeight":"40px","fontSize":"14px","textOverflow":"ellipsis","borderStyle":"dotted"}' class="name">{{item.dongmanmingcheng}}</div>
			...
			layui.use(['form', 'layer', 'element', 'carousel', 'laypage', 'http', 'jquery','laydate', 'slider'], function() {
				...
				http.request('config/list', 'get', {
        			...
        			http.request(`option/shipinfenlei/shipinfenlei`,'get',{},(res)=>{
                			vue.shipinfenleiOptions = res.data
				...
				//类型搜索
				  ...
				  // 获取列表数据
				  http.request('option/shipinfenlei/shipinfenlei', 'get', {}, function(res) {
				...
				if(category!=null && category!='全部') {
					vCategory = category;
					if (jquery('#dongmanmingcheng').val()) {
						param['dongmanmingcheng'] = '';
					}
					if (jquery('#dongmanzuozhe').val()) {
						param['dongmanzuozhe'] = '';
					}
				}
				if(vCategory!='' && category!='全部') {
					param['shipinfenlei'] = category;
				}

			        if (jquery('#dongmanmingcheng').val()) {
			          param['dongmanmingcheng'] = jquery('#dongmanmingcheng').val() ? '%' + jquery('#dongmanmingcheng').val() + '%' : '';
			        }
			        if (jquery('#dongmanzuozhe').val()) {
			          param['dongmanzuozhe'] = jquery('#dongmanzuozhe').val() ? '%' + jquery('#dongmanzuozhe').val() + '%' : '';
			        }
			        param['sort'] = 'faburiqi';
			        param['order'] = 'desc';
					...
					// 获取列表数据
					http.request('remendongman/list', 'get', param, function(res) {
